...
SELECT 'Member_' || g, '010-' || LPAD((random()*9999)::int::text, 4, '0') || '-' || LPAD((random()*9999)::int::text, 4, '0'),
	CURRENT_DATE - ((random()*1000)::int) FROM generate_series(1, 100000) g;

INSERT INTO members2(name, phone, join_date) VALUES
('홍길동', '010-1234-5678', CURRENT_DATE);

-- 검색 시간 확인하기 (INDEX 없이)
EXPLAIN ANALYZE SELECT * FROM members2 WHERE name = '홍길동';
-- Planning Time: 0.066 ms
-- Execution Time: 4.407 ms

-- INDEX 추가
CREATE INDEX idx_members_name2 ON members2(name);

-- 검색 시간 확인하기 (INDEX 있을 때)
EXPLAIN ANALYZE SELECT * FROM members2 WHERE name = '홍길동';
-- Planning Time: 0.066 ms -> Planning Time: 1.141 ms
-- Execution Time: 4.407 ms -> Execution Time: 0.096 ms

-- VIEW 추가
CREATE VIEW registration_view AS SELECT m.name AS 회원명,
	c.class_name AS 강좌명, r.register_data AS 신청일 FROM registrations r
	JOIN members m ON r.member_id = m.member_id
	JOIN classes c ON r.class_id = c.class_id;