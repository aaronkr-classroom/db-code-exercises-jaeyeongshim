"""
Customer: 고객(id(PK), 이름, 휴대폰, 주소, 등급코드(FK)) 
Customer_Grade_Code: 고객등급코드(_코드_, 등급, 기준) 
...
-- JOIN
SELECT e.emp_id, e.name, j.job_name, e.salary FROM employees AS e
JOIN job_code AS j ON e.job_code = j.code;
...
-- JOIN
SELECT c.id, c.name, c.phone, g.grade, g.criteria FROM customers c
JOIN customer_grade_code g ON c.grade_code = g.code;

-- Tour + Manager JOIN
SELECT t.tour_id, t.destination_location, t.program, e.name AS manager_name FROM tour_products t
JOIN employees e ON t.manager_emp_id = e.emp_id;

-- WHERE JOIN
SELECT t.program, t.tour_fee, e.name FROM tour_products t
JOIN employees e ON t.manager_emp_id = e.emp_id WHERE t.tour_fee >= 400000;

-- ORDER BY
SELECT name, salary FROM employees ORDER BY salary DESC;

-- Multiple JOINS
SELECT t.program, t.destination_location, e.name AS manager, j.job_name FROM tour_products t
JOIN employees e ON t.manager_emp_id = e.emp_id 
JOIN job_code j ON e.job_code = j.code;