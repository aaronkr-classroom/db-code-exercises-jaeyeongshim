...
1. 가장 최근에 생성된 현재 시뭔스 값을 반환하는 의사열 -- CURRVAL
2. 시권스를 변경하는 명령 -- ALTER SEQUENCE
3. 시뭔스를 삭제하는 명렁 -- DROP SEQUENCE
4. 사용자가 수동으로 인텍스를 생성하는 명렁 -- CREATE INDEX
10. 테이블 구조는 남겨두고 저장된 데이터만 삭제하는 명령 -- TRUNCATE TABLE
12. 기본 테이블의 구조를 변경하는 명령 -- ALTER TABLE
13. 기본 테이블로부터 만들어지는 가상 테이블인 뷰를 생성하는 영령 -- CREATE VIEW
14. 기본 테이블 구조와 함께 저장된 데이터와 인텍스까지 한꺼번에 삭제하는 영령 -- DROP TABLE
15. DDL 문으로 만들어지지 않고， 질의문 처리 괴정의 중간 결과로 만들어지는 테이블 -- 임시 테이블
16. 기존 테이블에 새로운 데이터를 삽입하는 명렁 -- INSERT
19. 시뭔스에 의해 지동으로 생성되는 가상의 열 -- 의사열
20. CREATE VIEW 명령을 이용해서 기몬 테이블로부터 만들어지는 테이블 -- 가상 테이블
22. 단 하나의 테이블만 기초로 하여 생성된 뷰 -- 단순 뷰
23. 테이블에서 기존 데이터를 갱신히는 명령 -- UPDATE 

**세로**

1. 시뭔스를 생성히는 명령 -- CREATE SEQUENCE
5. 기존 뷰를 삭제하는 영렁 -- DROP VIEW 
6. 기존 테이블에 새로운 데이터를 삽입하는 명령 -- INSERT
7. 테이블에서 기존 데이터를 삭제하는 명령 -- DELETE
8. 기본 테이블을 생성하는 명령 -- CREATE TABLE
9. 다음에 사용 가능한 시뭔스 값을 생성하는 의사열 -- NEXTVAL
11. 고정길이 문자열을 기억히는 데이터 타입 -- CHAR
17. 테이블에서 데이터를 검색하는 명렁 -- SELECT
...
'''SQL
ALTER TABLE lab ADD COULUM lab_size NUMERIC(4) DEFAULT 50;
'''
4. 02번 문제에서 생성한 lab 테이블의 ‘호실 (rooffi_id) ’ 열에 rooffi_id_idx라는 인텍스를 생성하는 SQL 문을 작성하시오.
'''SQL
CREATE INDEX room_id_idx ON lab(room_id);
'''
...
'''SQL
INSERT INTO lab (lab_num, name, building, room_id, dept_id) VALUES 
  (188, '가상현실', '2공학관', 'B203', 'comp'),
  (118, '인공지능', '2공학관', 'A101', 'comp');
'''
6. 02번 문제에서 생성한 1ab 테이블에 건물명이 ‘2공학관’인 모든 실험실의 명칭과 소속학과변호를 검색하는 SQL 문을 작성하시오.
'''SQL
SELECT name, dept_id FROM lab WHERE building = '2공학관';
'''
7. 02번 문제에서 생성한 1ab 데이블에 명칭이 ’인공지능’인 실험실의 호실을 ‘B102’로 변경하는 SQL 문을 작성하시오.
'''SQL
UPDATE lab SET room_id = 'B102' WHERE name = '인공지능';
'''
...
'''SQL
CREATE VIEW com_lab_view AS SELECT name, room_id, lab_size FROM lab WHERE dept_id = 'comp';
'''
...
VALUES (nextval('lab_num_seq'), '네트워크', '2공학관', 'B201', 'comp');
'''
